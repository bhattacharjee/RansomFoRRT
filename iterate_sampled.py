...
NUM_SAMPLES = 4
SAMPLE_BLOCK_SIZE = 256

MIN_FILE_SIZE = NUM_SAMPLES * SAMPLE_BLOCK_SIZE * 2
MIN_FILE_SIZE = min(2048, MIN_FILE_SIZE)

FORCE_RECALCULATION = False
FORCE_FOURIER_RECALCULATION = False

def sample_buffer(inbuffer, n_samples, block_size):
    while len(inbuffer) < MIN_FILE_SIZE:
        inbuffer = inbuffer + inbuffer
    out_buffer = b''
    max_index = len(inbuffer) - block_size - 1
    assert(max_index > 0)
    for i in range(n_samples):
        start_idx = random.randint(0, max_index)
        out_buffer += inbuffer[start_idx:start_idx+block_size]
    return inbuffer
...
def get_filesize(filename, bytearray, doublearray, metadata):
    ...
    if not "filesize" in metadata["baseline"] or FORCE_RECALCULATION:
        metadata["baseline"]["filesize"] = os.path.getsize(filename)
...
def get_fourier_psd(filename, byte1_array, doublearray, metadata):
    ...
    if "fourier" not in metadata \
        ...
        with open(filename, "rb") as fp:
            ...
            byte1_array = sample_buffer(byte1_array, NUM_SAMPLES, SAMPLE_BLOCK_SIZE)
...
def process_single_file(filename):
    ...
    try:
        with open(filename, "rb") as f:
            ...
            nparray = sample_buffer(nparray, NUM_SAMPLES, SAMPLE_BLOCK_SIZE)
